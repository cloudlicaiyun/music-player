<template>
  <transition name="list-fade">
    <div class="playlist" v-show="showFlag" @click="hide">
        ...
        <scroll ref="listContent" class="list-content" :refreshDelay="refreshDelay" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li class="item" ref="listItem" v-for="(item, index) in sequenceList" :key="item.id" @click="selectItem(item, index)">
              <i class="current" :class="getCurrentCl(item)"></i>
              <span class="text">{{item.name}}</span>
              <span class="like" @click.stop="toggleFavoriate(item)">
                <i :class="getFavoriateIcon(item)"></i>
              </span>
      ...
      <confirm ref="confirm" @confirm="confirmClear" @cancel="confirmCancel" text="是否清空歌单列表" confirmBtnText="清空"></confirm>
